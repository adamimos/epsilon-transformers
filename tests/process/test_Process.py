...
def test_msp_creation():
    ...
    assert paths_and_probs[0][0] == [] and np.allclose(paths_and_probs[0][1], np.array([1/3, 1/3, 1/3]))
    def _query_path(list_of_tuples, path: list) -> Tuple[List[int], np.ndarray]:
        for tup in list_of_tuples:
            if path == tup[0]:
                return tup
        raise ValueError(f"{path} not in list of tuples")
    assert np.allclose(_query_path(paths_and_probs, [])[1], np.array([1/3, 1/3, 1/3]))
    assert np.allclose(_query_path(paths_and_probs, [0])[1], np.array([1/3, 2/3, 0]))
    assert np.allclose(_query_path(paths_and_probs, [1])[1], np.array([1/3, 0, 2/3]))
    assert np.allclose(_query_path(paths_and_probs, [1, 0])[1], np.array([1/2, 1/2, 0]))
