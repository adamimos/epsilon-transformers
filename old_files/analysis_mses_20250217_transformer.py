...
def get_layer_mses_for_activations(acts: torch.Tensor, ground_truth_beliefs: torch.Tensor, word_probs: torch.Tensor) -> list[float]:
    """
    ...
    """
    n_layers, n_batch, n_ctx, d_model = acts.shape
    mses = []

    # Each layer
    for i in range(n_layers):
        mse_i = run_activation_to_beliefs_regression_fast_weighted(acts[i], ground_truth_beliefs, word_probs, train_samples=10000)
        mses.append(float(mse_i))

    # "All-layer" concat
    all_layers_acts = acts.permute(1, 2, 3, 0).reshape(n_batch, n_ctx, -1)
    mse_concat = run_activation_to_beliefs_regression_fast_weighted(all_layers_acts, ground_truth_beliefs, word_probs, train_samples=10000)
...
def random_init_control_experiment(
    ...
    for seed in tqdm(range(n_random_inits), desc="Random seeds"):
        ...
        # 3) MSE per layer
        mses_this_seed = get_layer_mses_for_activations(r_acts, nn_beliefs, nn_word_probs)
        # Length: n_layers+1
...
for run_id in runs:
    ...
    df_random_norm = random_init_control_experiment(
        transformer_config, nn_inputs, nn_beliefs, nn_word_probs, n_seeds
    )
    ...
    df_random_unnorm = random_init_control_experiment(
        transformer_config, nn_inputs, nn_unnormalized_beliefs, nn_word_probs, n_seeds
    )
    ...
    for ckpt in tqdm(all_ckpts, desc="Checkpoints"):
        ...
        mses_norm = get_layer_mses_for_activations(acts_ckpt, nn_beliefs, nn_word_probs)
        mses_unnorm = get_layer_mses_for_activations(acts_ckpt, nn_unnormalized_beliefs, nn_word_probs)
        print(f"{mses_norm=}")
        print(f"{mses_unnorm=}")
        ...
        mses_norm_shuffled = get_layer_mses_for_activations(acts_ckpt_shuffled, nn_beliefs, nn_word_probs)
        mses_unnorm_shuffled = get_layer_mses_for_activations(acts_ckpt_shuffled, nn_unnormalized_beliefs, nn_word_probs)
        for layer_idx in range(n_layers+1):
...
# %%
# Create test dataset for regression
def make_test_dataset():
    n_samples = 1000
    n_features = 64  # Typical d_model size
    n_targets = 40    # Typical belief dimension
    
    # Create input features - one meaningful dimension, rest noise
    # X = torch.randn(n_samples, n_features)
    X = torch.zeros(n_samples, n_features)
    
    # Make first dimension a perfect line
    true_signal = torch.linspace(-2, 2, n_samples)
    X[:, 0] = true_signal
    
    # Create target with first dimension being the line plus noise
    Y = torch.zeros(n_samples, n_targets)
    Y[:, 0] = true_signal #+ 0.1 * torch.randn(n_samples)
    # Other dimensions are pure noise
    Y[:, 1:] = torch.randn(n_samples, n_targets-1)
    
    # Create sample weights
    weights = torch.ones(n_samples)
    
    # add false dim to X and Y
    X = X.unsqueeze(0)
    Y = Y.unsqueeze(0)
    weights = weights.unsqueeze(0)
    # Test the regression
    mse = run_activation_to_beliefs_regression_fast_weighted(
        X, Y, None, train_samples=None
    )
    
    print(f"MSE on test dataset: {mse:.5f}")
    # Should be close to 0.01 (from the 0.1 std noise we added)


    # now compare this to sklearn linear regression
    from sklearn.linear_model import LinearRegression
    reg = LinearRegression().fit(X.squeeze().numpy(), Y.squeeze().numpy())
    print(f"Sklearn MSE: {((reg.predict(X.squeeze().numpy()) - Y.squeeze().numpy())**2).mean():.5f}")
