...
def main():
    ...
    triangle_vertices = np.array([[0, 0, 0], [0.5, np.sqrt(3) / 2, 0], [1, 0, 0], [0, 0, 0]])
    ax.plot(triangle_vertices[:,0], triangle_vertices[:,1], triangle_vertices[:,2], "k-", linewidth=0.5)  # Adjusted line width for crispness
    
    # scatter points in 3D space for the first triangle
    ax.scatter(simplex_points[0], simplex_points[1], np.zeros_like(simplex_points[0]), c="black", marker="o", s=0.5)  # Adjusted marker size for visibility
    ...
    def calculate_rotation_matrix(point_on_line, direction_of_line, rotation_angle):
        ...
        direction_of_line_normalized = direction_of_line / np.linalg.norm(direction_of_line)
        a, b, c = direction_of_line_normalized
        d, e, f = point_on_line
        t = rotation_angle
        st, ct = np.sin(t), np.cos(t)
        return np.array([
            [ct + a**2 * (1 - ct), a * b * (1 - ct) - c * st, a * c * (1 - ct) + b * st, (d * (b**2 + c**2) - a * (e * b + f * c)) * (1 - ct) + (e * c - f * b) * st],
            [a * b * (1 - ct) + c * st, ct + b**2 * (1 - ct), b * c * (1 - ct) - a * st, (e * (a**2 + c**2) - b * (d * a + f * c)) * (1 - ct) + (f * a - d * c) * st],
            [a * c * (1 - ct) - b * st, b * c * (1 - ct) + a * st, ct + c**2 * (1 - ct), (f * (a**2 + b**2) - c * (d * a + e * b)) * (1 - ct) + (d * b - e * a) * st],
            [0, 0, 0, 1]
        ])

    point_on_line = np.array([0.5, np.sqrt(3) / 6, 0])  # This is the midpoint of the triangle base
    direction_of_line = np.array([1, 0, 0])  # This direction makes the line perpendicular to the triangle base
    rotation_matrix = calculate_rotation_matrix(point_on_line, direction_of_line, rotation_angle)

    # Apply the rotation
    homogenous_triangle_vertices = np.hstack((triangle_vertices, np.ones((triangle_vertices.shape[0], 1))))
    # Correcting the subtraction operation to ensure broadcasting compatibility
    rotated_triangle_vertices_homogenous = np.dot(homogenous_triangle_vertices - np.hstack((point_on_line, [1])), rotation_matrix.T) + np.hstack((point_on_line, [1]))
    rotated_triangle_vertices = rotated_triangle_vertices_homogenous[:, :3]
    ax.plot(rotated_triangle_vertices[:,0], rotated_triangle_vertices[:,1], rotated_triangle_vertices[:,2], "k-", linewidth=0.5)
    ...
    rotated_simplex_points = np.dot(np.vstack((simplex_points[0], simplex_points[1], np.zeros_like(simplex_points[0]), np.ones_like(simplex_points[0]))).T - np.hstack((point_on_line, [1])), rotation_matrix.T) + np.hstack((point_on_line, [1]))
    rotated_simplex_points = rotated_simplex_points[:, :3]  # Discarding the homogenous coordinate
    ax.scatter(rotated_simplex_points[:,0], rotated_simplex_points[:,1], rotated_simplex_points[:,2], c="black", marker="o", s=0.5)
    # add an xyz axis
    ...
    rotation_matrix1 = calculate_rotation_matrix(point_on_line, direction_of_line, rotation_angle1)

    rotated_triangle1_vertices = np.dot(np.hstack((triangle_vertices[:, :3], np.ones((triangle_vertices.shape[0], 1)))), rotation_matrix1.T)
    triangle1 = pv.PolyData(rotated_triangle1_vertices[:, :3], np.hstack([[len(cells)], cells]))

    if isinstance(simplex_points, tuple):
        simplex_points = np.array(simplex_points)  # Assuming simplex_points is structured as (x_points, y_points)
    ...
    rotated_simplex_points1 = np.dot(np.hstack((simplex_points_3d, np.ones((simplex_points_3d.shape[0], 1)))), rotation_matrix1.T)  # Removed unnecessary addition/subtraction for translation
    rotated_simplex_points1 = rotated_simplex_points1[:, :3]  # Discarding the homogenous coordinate

    # Add the first triangle and scatter points to the plot
    plotter.add_mesh(triangle1, color='#5F9EA0', show_edges=True, line_width=1, opacity=1,
    ...
    rotated_triangle_vertices = np.dot(np.hstack((triangle_vertices[:, :3], np.ones((triangle_vertices.shape[0], 1)))), rotation_matrix.T)

    # Create a PolyData object for the rotated triangle
    triangle2 = pv.PolyData(rotated_triangle_vertices[:, :3], np.hstack([[len(cells)], cells]))

    # Add the rotated triangle to the plot with a more reddish shade
    plotter.add_mesh(triangle2, color='#D2691E', show_edges=True, line_width=1, opacity=1,
                        specular=0.5, specular_power=15, smooth_shading=True)
    ...
    new_camera_position = (camera_position[0][0] + 1, camera_position[0][1] + 1, camera_position[0][2] + 1)
    plotter.camera_position = (new_camera_position, camera_position[1], camera_position[2])
