...
def _weighted_r2_transfer(X, Y_fit, Y_eval, weights, rcond=1e-10):
    """Fit regression on Y_fit, evaluate R² on Y_eval.

    Uses the full dataset (no CV) since we're testing transfer, not
    generalization of the fit itself.

    Parameters
    ----------
    X : ndarray, shape (N, d_features)
    Y_fit : ndarray, shape (N, d_targets)  — targets used to fit beta
    Y_eval : ndarray, shape (N, d_targets) — targets used to evaluate R²
    weights : ndarray, shape (N,)
    rcond : float

    Returns
    -------
    r2 : float — R² of the Y_fit-fitted model evaluated on Y_eval
    """
    N = X.shape[0]
    w = weights / weights.sum()
    ...
    # Fit on Y_fit
    beta, _, _, _ = np.linalg.lstsq(
        X_bias * sqrt_w, Y_fit * sqrt_w, rcond=rcond
    )

    # Evaluate on Y_eval
    Y_pred = X_bias @ beta
    Y_mean = (Y_eval * w[:, None]).sum(axis=0)
    ss_res = (w[:, None] * (Y_eval - Y_pred) ** 2).sum()
    ss_tot = (w[:, None] * (Y_eval - Y_mean) ** 2).sum()
