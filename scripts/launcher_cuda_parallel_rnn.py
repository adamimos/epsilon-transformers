...
def main():
    ...
    while experiment_queue or running_processes:
        while experiment_queue and available_gpus:
            ...
            experiment_name = f"run_{run_counter}_L{run_cfg['model_config']['num_layers']}_H{run_cfg['model_config']['hidden_size']}_{run_cfg['model_config']['rnn_type']}_{'bi' if run_cfg['model_config'].get('bidirectional', False) else 'uni'}_{run_cfg['process_config']['name']}"
