...
for artifact in artifacts:
    print(
        f"Artifact Name: {artifact.name}, Type: {artifact.type}, Version: {artifact.version}"
    )
...
with torch.no_grad():
    for i in range(0, X_val.size(0), batch_size):
        ...
        loss = F.cross_entropy(
            logits_flat, Y_val_flat, reduction="none"
        )  # [batch_size * n_ctx]
        # Reshape loss to original [batch_size, n_ctx] to multiply by val_weights
        loss = loss.view(
            X_val_batch.shape[0], X_val_batch.shape[1]
        )  # [batch_size, n_ctx]
        # Apply weights
        val_weights_repeat = val_weights_batch.unsqueeze(1).repeat(
