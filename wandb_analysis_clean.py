...
def generate_belief_state_figures_datashader(
    belief_states, all_beliefs, predicted_beliefs, plot_triangles=False
):
    ...
    agg_gt = {
        color: cvs.points(df_gt, "x", "y", agg_funcs[color])
        for color in ["r", "g", "b"]
    }
    agg_pb = {
        color: cvs.points(df_pb, "x", "y", agg_funcs[color])
        for color in ["r", "g", "b"]
    }
    ...
    fig, axs = plt.subplots(
        1, 2, figsize=(10, 5), sharex=True, sharey=True, facecolor="white"
    )
    for ax in axs:
...
def generate_belief_state_figures(
    ...
    axs[0].scatter(x_gt, y_gt, c=colors_gt, s=s, alpha=1.0)
...
for art in tqdm(arts):
    ...
    def analyze_artifact(
        ...
        fig = generate_belief_state_figures_datashader(
            belief_states, all_beliefs_reshaped, predicted_beliefs, plot_triangles=True
        )
...
belief_state_images = [
    img
    for img in os.listdir()
    if img.startswith("belief_states_") and img.endswith(".png")
]
belief_state_images.sort(
