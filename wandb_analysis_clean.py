...
def generate_belief_state_figures_datashader(belief_states, all_beliefs, predicted_beliefs, plot_triangles=False):
    ...
    df_pb = pd.DataFrame({'x': pb_x, 'y': pb_y, 'r': predicted_beliefs[:, 0], 'g': predicted_beliefs[:, 1], 'b': predicted_beliefs[:, 2]})

    # Create canvas
    cvs = ds.Canvas(plot_width=500, plot_height=500, x_range=(-0.1, 1.1), y_range=(-0.1, np.sqrt(3)/2 + 0.1))
    ...
    def combine_channels_to_rgb(agg_r, agg_g, agg_b):
        img_r = tf.shade(agg_r, cmap=['white', 'red'], how='linear')
        img_g = tf.shade(agg_g, cmap=['white', 'green'], how='linear')
        img_b = tf.shade(agg_b, cmap=['white', 'blue'], how='linear')
    ...
    fig.text(0.5, title_y_position, 'Ground Truth', ha='center', va='top', transform=axs[0].transAxes, color='black', fontsize=15)
    fig.text(0.5, title_y_position, 'Residual Stream', ha='center', va='top', transform=axs[1].transAxes, color='black', fontsize=15)

        
    if plot_triangles:
        for ax in axs:
            ax.plot([0, 0.5, 1, 0], [0, np.sqrt(3)/2, 0, 0], 'black', lw=2)
