...
if config["process"] == "RRXOR":
    ...
    process = mess3()
elif config["process"] == "zero_one_rßßandom":
    process = zero_one_random()
elif config["process"] == "zero_one_random_abstracted":
    train_process = zero_one_random_set()
    abstract_process = zero_one_random_abstracted()
...
abstract_minimum_cross_entropy = abstract_myopic_entropy_rate[1:]

print(f"myopic_entropy_rate: {myopic_entropy_rate}")
print(f"abstract_myopic_entropy_rate: {abstract_myopic_entropy_rate}")
...
def sweep_train(config: Optional[Dict] = None):
    with wandb.init(config=config):
        ...
        X_val, Y_val, val_weights = create_validation_set(train_MSP_tree, config.n_ctx)
        ...
        for epoch in range(config.num_epochs):
            ...
            train_epoch_prob(
                ...
                abstract_val_data,
                Y_abstract_val,
                abstract_val_weights,
                abstract_minimum_cross_entropy,
            )
...
def train_epoch_prob(
    ...
    epcoh_ind: int,
    X_abstract_val: torch.Tensor,
    Y_abstract_val: torch.Tensor,
    abstract_val_weights: torch.Tensor,
    abstract_minimum_cross_entropy: torch.Tensor,
):
    model.train()
    for weights in train_weights:
        ...
        loss = loss.view(val_data.shape[0], val_data.shape[1])  # *(batch_size, n_ctx)

        mean_loss, relative_loss = compute_val_losses(
            loss, minimum_cross_entropy, weights
        )
    ...
    with torch.no_grad():
        ...
        loss = criterion(Y.view(-1, model.cfg.d_vocab), Y_abstract_val.view(-1))
        loss = loss.view(X_abstract_val.shape[0], X_abstract_val.shape[1])
        mean_loss, relative_loss = compute_val_losses(
