...
def transition_matrix_to_graph(transition_matrix: np.ndarray, state_names: Optional[Dict[str, int]] = None) -> nx.DiGraph:
    """
    Convert a transition matrix to a graph.

    Parameters:
    transition_matrix (np.ndarray): The transition matrix of shape (n_states, n_states).
                                    n_states is the number of states in the machine.
                                    transition_matrix[j, k] is the probability of transitioning from state j to state k.
