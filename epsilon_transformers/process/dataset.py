...
class ProcessDataset(IterableDataset):
    ...
    def __init__(self, process_name: str, sequence_length: int, num_samples: int):
        ...
        if process is None:
            raise ValueError(f"{process_name} is not a recognized process. It must be one of the following {PROCESS_REGISTRY.keys()}")
        
        self.samples = process.yield_emissions(sequence_len=num_samples * (sequence_length + 1))
    ...
    def __iter__(self) -> Iterable[Tuple[List[int]]]:
        for _ in range(self.num_samples):
            process_history = [next(self.samples) for _ in range(self.sequence_length + 1)]
