...
class MixedStateTree:
	...
	def block_entropy(self) -> Float[np.ndarray, "depth"]:
		...
		def traverse(node: MixedStateTreeNode, depth: int, accumulated_prob: float):
			if depth < self.depth:
				if node is not self.root_node:
					depth_emission_probs[depth].append(accumulated_prob * node.emission_prob)
				for child in node.children:
					traverse(child, depth + 1, accumulated_prob * node.emission_prob if node is not self.root_node else 1.0)
		
		traverse(self.root_node, 0, 1.0)
